...
int main(int argc, char *argv[]){
    ...
    if(mode == "count"){
        ...
        offlineCountMinSketchObject.createOfflineCountMinSketch(height, width, total_round, conservative, canonicalize);

        std::chrono::steady_clock::time_point begin = std::chrono::steady_clock::now();
        offlineCountMinSketchObject.constructOfflineCountMinSketch(input_fasta_file,kmer_len);
        ...
        OfflineCountMinSketch <uint64_t, WangHash> offlineCountMinSketchObject;
        offlineCountMinSketchObject.createOfflineCountMinSketch(input_sketch_name);
        ...
        while (infile >> kmer){
            query_result_file <<kmer<<","<<offlineCountMinSketchObject.estimate_count(kmer)<<endl;
        }
    }
